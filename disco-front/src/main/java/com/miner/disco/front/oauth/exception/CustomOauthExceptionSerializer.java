...
public class CustomOauthExceptionSerializer extends StdSerializer<CustomOauthException> {
    ...
    public void serialize(CustomOauthException value, JsonGenerator gen, SerializerProvider provider) throws IOException {
        ...
        if (StringUtils.equals("Bad credentials", message)) {
            message = "用户名或密码错误";
            status = BusinessExceptionCode.USERNAME_OR_PASSWORD_ERROR.getCode();
        }
//        if(StringUtils.equals(""))

        gen.writeStartObject();
        gen.writeStringField("message", message);
        gen.writeStringField("status", String.valueOf(BusinessExceptionCode.USERNAME_OR_PASSWORD_ERROR.getCode()));
