...
public class IdentifierService {
    ...
    // 基准时间
    private long twepoch = 1547014470000L;

    // 区域标志位数
    private final static long regionIdBits = 3L;

    // 机器标识位数
    private final static long workIdBits = 10L;
    ...
    // 业务ID偏左移20位
    private final static long regionIdShift = sequenceBits + workerIdShift;
    // 时间毫秒左移23位
    private final static long timestampLeftShift = sequenceBits + workerIdShift + regionIdBits;
    ...
    public IdentifierService(long workerId, long regionId) {

        // 如果超出范围就抛出异常
        if (workerId > maxWorkerId || workerId < 0) {
            throw new IllegalArgumentException("work Id can't be greater the %d or less than 0");
    ...
    public IdentifierService(long workerId) {
        // 如果超出范围就抛出异常
        if (workerId > maxWorkerId || workerId < 0) {
